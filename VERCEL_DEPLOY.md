...
V13.4.3부터 오류는 기본적으로 Vercel Function Logs에 구조화 JSON으로 기록됩니다.
장기 보관이 필요하면 PostgreSQL 연결 문자열을 환경변수로 추가하세요.

```text
DATABASE_URL=postgresql://...
```

설정 시 `app_error_logs` 테이블이 자동 생성됩니다. 수동 생성 스키마는 `LOGGING_DB_SCHEMA.sql`에 있습니다.
낮은 신뢰도 결과까지 운영 분석용으로 저장하려면 `LOG_LOW_CONFIDENCE=1`을 추가합니다.
